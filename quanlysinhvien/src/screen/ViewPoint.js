...
import Point from './Point';
 ...
 export default class ViewPoint extends React.Component {
  static navigationOptions = ({ navigation }) => {
		return {
		  title: navigation.getParam('otherParam', 'Danh Sách Điểm '),
		  headerRight: (
    ...
    componentDidMount(){
      return fetch('http://192.168.244.2:4000/diem/viewdiem')
    ...
    onPressAdd=()=> {
        this.props.navigation.dispatch(StackActions.reset({
            index: 0,
            actions: [
                NavigationActions.navigate({ routeName: 'AddPoint' })
            ],
            }))
    };
    // Hàm thoat 
          ...
          <View style={{ flex: 1 }}>
              <View style={{ flex: 1 }}>
                <View style={[styles.flexdong]}>
                  ...
                  <Text style={styles.tieude}> DANH SÁCH ĐIỂM </Text>
            ...
            <View>
                ...
                <FlatList
                      ...
                      renderItem={({item}) => <Point Masv = {item.masv} Mamon = {item.mamon} Lythuyet = {item.lythuyet}  Thuchanh = {item.thuchanh} Kiemtra = {item.kiemtra}  Tbmon = {item.tbmon} />}
